"""BLS CPI news-release archive scraper."""
...
_ARCHIVE_URL = "https://www.bls.gov/bls/news-release/cpi.htm"
_CURRENT_PDF = "https://www.bls.gov/news.release/pdf/cpi.pdf"
_CURRENT_HTML = "https://www.bls.gov/news.release/cpi.nr0.htm"
_HEADERS = {
...
_ARCHIVE_HREF_RE = re.compile(
    r"^/news\.release/archives/cpi_(?P<m>\d{2})(?P<d>\d{2})(?P<y>\d{4})\.pdf$",
    re.IGNORECASE,
)
...
def scrape_archive() -> tuple[dict[str, Any], ...]:
    """Return one entry per archived CPI release PDF, sorted newest-first."""
    import requests
    ...
    for href, title in titles_by_pdf.items():
        ...
        out.append(
            {
                "date": release_date,
                "title": title or f"CPI release {release_date.isoformat()}",
...
def current_release() -> dict[str, Any]:
    """Return the always-current CPI release entry (stable URL, no date)."""
    return {
