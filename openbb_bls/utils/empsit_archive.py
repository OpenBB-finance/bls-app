"""BLS Employment Situation (EMPSIT) news-release archive scraper."""
...
_ARCHIVE_URL = "https://www.bls.gov/bls/news-release/empsit.htm"
_CURRENT_PDF = "https://www.bls.gov/news.release/pdf/empsit.pdf"
_CURRENT_HTML = "https://www.bls.gov/news.release/empsit.nr0.htm"
_HEADERS = {
...
_ARCHIVE_HREF_RE = re.compile(
    r"^/news\.release/archives/empsit_(?P<m>\d{2})(?P<d>\d{2})(?P<y>\d{4})\.pdf$",
    re.IGNORECASE,
)
...
def scrape_archive() -> tuple[dict[str, Any], ...]:
    """Return one entry per archived Employment Situation release PDF, sorted newest-first."""
    import requests
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        # stale (several recent Employment Situation entries share one
        # outdated aria-label) while the list text carries the correct month.
        title = ""
        ...
        out.append(
            {
                "date": release_date,
                "title": title
                or f"Employment Situation release {release_date.isoformat()}",
...
def current_release() -> dict[str, Any]:
    """Return the always-current Employment Situation release entry (stable URL)."""
    return {
