"""BLS U.S. Import and Export Price Indexes (XIMPIM) news-release archive scraper."""
...
_ARCHIVE_URL = "https://www.bls.gov/bls/news-release/ximpim.htm"
_CURRENT_PDF = "https://www.bls.gov/news.release/pdf/ximpim.pdf"
_CURRENT_HTML = "https://www.bls.gov/news.release/ximpim.nr0.htm"
_HEADERS = {
...
_ARCHIVE_HREF_RE = re.compile(
    r"^/news\.release/archives/ximpim_(?P<m>\d{2})(?P<d>\d{2})(?P<y>\d{4})\.pdf$",
    re.IGNORECASE,
)
...
def scrape_archive() -> tuple[dict[str, Any], ...]:
    """Return one entry per archived XIMPIM release PDF, sorted newest-first."""
    import requests
    ...
    for href, title in titles_by_pdf.items():
        ...
        out.append(
            {
                "date": release_date,
                "title": title or f"XIMPIM release {release_date.isoformat()}",
...
def current_release() -> dict[str, Any]:
    """Return the always-current XIMPIM release entry (stable URL, no date)."""
    return {
