...
_STATE_TABLE_OFFSET = 100


def _decode_table(value: int) -> tuple[str, int]:
    """Split an encoded dropdown value into a ``(scope, table_number)`` pair.

    State tables are offset by ``_STATE_TABLE_OFFSET`` in the dropdown so the
    single ``table_number`` control carries both the scope and the number
    (e.g. ``101`` -> ``("state", 1)``).
    """
    if value > _STATE_TABLE_OFFSET:
        return "state", value - _STATE_TABLE_OFFSET
...
class BlsJoltsChangeAnalysisQueryParams(QueryParams):
    ...
    table_number: int = Field(
        ...
        le=112,
        description=(
            "Numbered change-analysis table to load. National tables are 1-12; "
            "state tables are offset by 100 (e.g. 101 = State Table 1)."
        ),
    )
    ...
    def extract_data(
        ...
        **kwargs: Any,
        ...
        if table_number not in scope_map:
            raise OpenBBError(
                f"JOLTS {scope} table {table_number} does not "
                f"exist (valid range: {min(scope_map)}-{max(scope_map)})."
            )
        content = fetch_change_analysis_txt(scope, table_number)
        return parse_change_analysis(content, scope, table_number)
        ...
        **kwargs: Any,
        ...
        if not rows:
            ...
            raise EmptyDataError(
                f"No rows parsed from BLS JOLTS {scope} table {table_number}."
            )
