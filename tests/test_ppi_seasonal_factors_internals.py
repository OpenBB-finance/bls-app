...
def _clear_caches():
    ...
    psf._fetch_html_table.cache_clear()
    yield
    psf._wps_titles.cache_clear()
    psf._fetch_html_table.cache_clear()
...
def test_fetch_html_table_seafac_branch(monkeypatch):
    ...
    assert all(r["label"] for r in result["rows"])
    wps011 = next(r for r in result["rows"] if r["code"] == "WPS011")
    assert wps011["label"] == "Farm products-Fruits & melons, fresh/dry vegs. & nuts"


def test_wps_titles_from_bundled_asset():
    """_wps_titles loads the bundled WPS code->title map."""
    titles = psf._wps_titles()
    assert titles
    assert titles["WPS011"] == "Farm products-Fruits & melons, fresh/dry vegs. & nuts"
    assert titles["WPS00000000"] == "All commodities"
    assert psf._wps_titles() is titles
